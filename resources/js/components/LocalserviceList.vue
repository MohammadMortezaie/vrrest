...
<template>
    <div>
        ...
        <a class="btn bg-vr text-white" href="/vr/localservice/create"> <i class="bi bi-plus"></i> Add New Local Service </a>


        <hr class="my-4" />
        <h3>Local Service List</h3>
        <table class="table table-striped">
            ...
            <tbody>
                <tr v-for="item in post_data" :key="item.id">
                    ...
                    <td>
                        <a type="button" class="btn btn-primary" :href="'/vr/localservice/' + item.id + '/edit'">
        ...
        methods: {
            ...
            axiosDelete(item) {
                if (confirm('Are you sure you want to Delete ' + item.title + " ?")) {
                    const url = window.location.origin + "/vr/localservice/" + item.id;
                    axios.defaults.withCredentials = true;
